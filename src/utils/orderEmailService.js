...
const generateOrderItemsHTML = (items) => {
  ...
  items.forEach((item) => {
    ...
    const imageUrl = item.image && item.image.startsWith('http') 
      ? item.image 
      : (item.image ? `http://localhost:5000${item.image}` : 'https://via.placeholder.com/60');
        ...
        <td style="padding: 15px 12px;">
          <div style="display: flex; align-items: center; gap: 20px;">
            <img src="${imageUrl}" alt="${item.productName}" 
                 style="width: 70px; height: 70px; object-fit: cover; border-radius: 10px; border: 1px solid ${BRAND_COLORS.border};">
            <div style="margin-left: 5px;">
              <strong style="color: ${BRAND_COLORS.text}; font-size: 15px;">${item.productName}</strong>
              ${item.discountPrice > 0 ? `<p style="margin: 6px 0 0 0; font-size: 12px; color: ${BRAND_COLORS.success};">🎉 Sale Price Applied</p>` : ''}
            </div>
          </div>
        </td>
        <td style="padding: 15px 12px; text-align: center; font-size: 14px;">${item.quantity}</td>
        <td style="padding: 15px 12px; text-align: right; font-size: 14px;">${formatPrice(price)}</td>
        <td style="padding: 15px 12px; text-align: right; font-weight: 600; color: ${BRAND_COLORS.primary}; font-size: 14px;">${formatPrice(totalPrice)}</td>
      </tr>
    `;
  });
  ...
  return `
    <div style="background: ${BRAND_COLORS.lightBg}; padding: 20px; border-radius: 12px; margin: 20px 0;">
      ...
      <table style="width: 100%; border-collapse: collapse;">
        ...
        ${order.discount > 0 ? `
        <tr>
          <td style="padding: 8px 0; color: ${BRAND_COLORS.success};"><strong>Discount:</strong></td>
          <td style="text-align: right; color: ${BRAND_COLORS.success};">-${formatPrice(order.discount)}</td>
        </tr>
        ${order.couponCode ? `
        <tr>
          <td style="padding: 8px 0;"><strong>Coupon Applied:</strong></td>
          <td style="text-align: right;">${order.couponCode}</td>
        </tr>
        ` : ''}
        ` : ''}
...
// };
/**
 * Generate delivery info HTML - Now shows for all statuses if there's a delivery note
 */
/**
...
const generateDeliveryInfoHTML = (order) => {
  ...
  let bgColor = BRAND_COLORS.lightBg;
  let borderColor = BRAND_COLORS.primary;
  let titleColor = BRAND_COLORS.primary;
  let titleIcon = '📝';
  
  if (order.orderStatus === 'delivered') {
    bgColor = '#ECFDF5';
    borderColor = BRAND_COLORS.success;
    titleColor = BRAND_COLORS.success;
    titleIcon = '✅';
  ...
  return `
    <div style="background: ${bgColor}; padding: 20px; border-radius: 12px; margin: 20px 0; border-left: 4px solid ${borderColor};">
      <h2 style="margin: 0 0 15px 0; color: ${titleColor}; font-size: 18px; display: flex; align-items: center; gap: 8px;">
        <span>${titleIcon}</span> <span>${order.orderStatus === 'cancelled' ? 'Cancellation Information' : 'Delivery Information'}</span>
      </h2>
      <table style="width: 100%; border-collapse: collapse;">
        ${hasCancellationReason ? `
        <tr>
          <td style="padding: 8px 0; width: 140px;"><strong>Cancellation Reason:</strong></td>
          <td><div style="background: white; padding: 12px; border-radius: 8px; margin-top: 5px; color: #EF4444;">${order.cancellationReason}</div></td>
        </tr>
        ` : ''}
        ${hasDeliveredDate ? `
        <tr>
        ...
        <tr>
          <td style="padding: 8px 0;"><strong>Delivery Note:</strong></td>
          <td><div style="background: white; padding: 12px; border-radius: 8px; margin-top: 5px;">${order.deliveryNote}</div></td>
        </tr>
        ` : ''}
      </table>
    </div>
...
const sendOrderNotificationToAdmin = async (order, eventType = 'new') => {
  ...
  try {
    ...
    if (eventType === 'new') {
      ...
      headerEmoji = '🛍️';
      headerColor = 'linear-gradient(135deg, #4A8A90 0%, #FFB6C1 100%)';
      additionalMessage = 'A new order has been placed and requires your attention.';
    } else if (eventType === 'status_update') {
      const statusInfo = {
        'confirmed': { title: 'Order Confirmed', emoji: '✅', color: 'linear-gradient(135deg, #10B981 0%, #34D399 100%)' },
        'processing': { title: 'Order Processing', emoji: '⚙️', color: 'linear-gradient(135deg, #8B5CF6 0%, #A78BFA 100%)' },
        'shipped': { title: 'Order Shipped', emoji: '🚚', color: 'linear-gradient(135deg, #3B82F6 0%, #60A5FA 100%)' },
        'delivered': { title: 'Order Delivered', emoji: '📦', color: 'linear-gradient(135deg, #10B981 0%, #34D399 100%)' },
        'cancelled': { title: 'Order Cancelled', emoji: '❌', color: 'linear-gradient(135deg, #EF4444 0%, #F87171 100%)' }
      };
      const info = statusInfo[order.orderStatus] || { title: 'Order Updated', emoji: '📝', color: 'linear-gradient(135deg, #4A8A90 0%, #FFB6C1 100%)' };
      ...
      const paymentInfo = {
        'paid': { title: 'Payment Received', emoji: '💰', color: 'linear-gradient(135deg, #10B981 0%, #34D399 100%)' },
        'failed': { title: 'Payment Failed', emoji: '⚠️', color: 'linear-gradient(135deg, #EF4444 0%, #F87171 100%)' },
        'refunded': { title: 'Payment Refunded', emoji: '💸', color: 'linear-gradient(135deg, #F97316 0%, #FBBF24 100%)' }
      };
      const info = paymentInfo[order.paymentStatus] || { title: 'Payment Updated', emoji: '💳', color: 'linear-gradient(135deg, #4A8A90 0%, #FFB6C1 100%)' };
      headerTitle = info.title;
      headerEmoji = info.emoji;
      headerColor = info.color;
      ...
      html: `
        ...
        <head>
          ...
          <style>
            body { font-family: 'Inter', Arial, sans-serif; line-height: 1.6; color: #333; background-color: ${BRAND_COLORS.lightBg}; }
            .container { max-width: 700px; margin: 20px auto; background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.1); }
            .header { background: ${headerColor}; padding: 25px 30px; text-align: center; }
            .header h1 { color: white; margin: 0; font-size: 24px; display: flex; align-items: center; justify-content: center; gap: 10px; }
            .content { padding: 30px; }
            .button { background: ${BRAND_COLORS.primary}; color: white; padding: 12px 30px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: bold; }
            .section-title { font-size: 18px; font-weight: 600; margin: 25px 0 15px 0; color: ${BRAND_COLORS.text}; }
          </style>
